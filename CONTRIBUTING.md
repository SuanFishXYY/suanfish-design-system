...
node scripts/charter-lint.mjs   # 五类检测：A 元数据一致性 / B 章程 frontmatter 契约 / C agent refs 悬空 / D ref 间引用网络 / **E 语义逻辑债（R8 E1 R编号空洞/R3 + E2 被审者审自己/R5 + E3 escape-hatch无防滥用/R7 + R12 E4 R-Cross编号空洞/R3同构）**。🟥 非零即拦；🟧 警告（填实对应 ref / 补防滥用词即消）。
```
