...
- **M5 Keeper Test**：审核者失准下岗动议（需 sage_congress 2/3 仲裁通过才执行）。**🔴 避嫌律（R5）**：被弹劾圣人本人 + 其师承链上下 1 跳强制回避不计分母；仲裁议会由三大类重新选拔未被牵连组（非原被审议会自审）；蓝军互攻弹劾同理互回避。避嫌后人数不足则扩召独立组补齐，不搁置不放过的。**🔴 扩召优先级（R6）**：先类别后人数——扩召须先保 G4 类别覆盖（S 级≥3 类/A 级≥2 类）再补人数，防哲学类（板凳最厚）独占致凑齐人数却只 1 类代表→凑不齐退回→弹劾不能退回→死循环；某类全员被牵连则记"缺类"降级处理不伪造覆盖（详见 ref 30 §M5 避嫌律）。
