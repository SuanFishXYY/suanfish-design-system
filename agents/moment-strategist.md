...
## 🛑 REJECT 触发表（v2.1 写死 · 不再依赖人脑）

只要命中任意一条，**必须** 输出 REJECT 而非 BRIEF：

| # | 条件 | 理由 |
| --- | --- | --- |
| R1 | 时长预算 > 5s **且** 非用户主动触发 | 强加体验 |
| R2 | 仪式装饰落在每天 100+ 次的高频界面 | 长期骚扰 |
| R3 | 需求自相矛盾（如「快速完成」+「沉浸仪式」） | 物理不可能 |
| R4 | 违反三条铁律之一 | 系统性破坏 |
| R5 | 想走路径 B 但 4 项硬条件未全满足 | 路径 B 已硬约束 |
| R6 | 一屏内要同时跑 onboarding 与 ui-architect 主导 | 双主导冲突 |
