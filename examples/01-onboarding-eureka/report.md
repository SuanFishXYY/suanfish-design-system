...
| 规则 | 状态 | 详情 |
| --- | --- | --- |
| （见 ref 16 规则集）仪式专用 token 命名空间 | ✅ | ceremonial-* 全部隔离 |
| W-01 入场动画总时长 | ✅ | 3 步 × 5s 上限 = 15s |
| R-04 跳过按钮 | ⚠️ | 跳过 + ESC（与 P-OD2 张力，见下注） |
| R-05 步骤数 | ✅ | 3 步骨架，未超 |
| ... | — | 15 条规则中 14 条通过 |

> ⚠️ R-04（禁跳过按钮）与 P-OD2（eureka 要求可跳过）张力：本例 onboarding-eureka 按 P-OD2 允许 skip，属 eureka 例外，待 ref 16 R-04 加 scope（v4.2.7 已加）
