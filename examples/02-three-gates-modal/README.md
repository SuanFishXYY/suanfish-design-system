...
> *演示 ref 28-30 端到端制度规范（非运行时执行体·执行待 v4.3）。*
> 一个故意触发退回 + 事后复审的模态设计 case，演示三道门全流程流转。
...
## demo 演示什么（制度规定·非运行时执行证据）

| 制度规定 | demo 演示 |
| --- | --- |
| ref 28 入口门拦违规 | G5 蓝军 R-05 否决，议会全票也过不了 |
| ref 29 退回门闭环整改 | R1-R5 五步走完，工单销项回 G3 复审 PASS |
| ref 30 事后门查衰弱 | K 轨复审判定存活/衰弱分支 |
| review-orchestrator 编排规范 | case_id 状态机流转（v4.2.7 手工演示·运行时待 v4.3） |
