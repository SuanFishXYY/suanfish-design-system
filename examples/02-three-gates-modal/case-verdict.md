...
## 指标采集（进 review-orchestrator 季度 dashboard）
...
✅ **入口门**拦住了违规（R-05 否决，议会全票也过不了）
✅ **退回门**闭环了整改（R1-R5 五步走完，工单销项回 G3 复审 PASS）
✅ **事后门**查出了衰弱（K 轨 3 月后复审发现 dark 适配缺失）
✅ **编排器**全程可追溯（case 状态机 + SLA + 指标齐全）
✅ **交叉喂食**生效（K 衰弱 → M2 判松严 → 确认规则集迭代合理）

> ref 28-30 不是 PDF，是能跑通的流水线。本 demo 即证据。
