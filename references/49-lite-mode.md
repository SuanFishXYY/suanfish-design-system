...
> *至尊版是"每单都开 420 板凳议会 + 三道门审核"——严谨但令牌重。Lite 模式借 sptler（孪生项目·见 README §孪生项目） 的四轨自动降级：日常单点问题 1 位设计师 3 句话裁决，复杂议题才开多专家链。小问题不背仪式税。*
...
>
> ⚠️ **v4.2.7 诚实声明（M7）**：本表为**估算**·实际至尊版 token 显著更高（三轮投票 30k+ per ref48§11/bench-matcher:429·板凳评分加载 ref27 227KB≈75k+）·~26k 预算低估约 4-5x·「省 85%」是相对值（vs 估算的至尊版）非绝对·精确预算待实测。
...
5. **轨道声明（v4.2.7 M2-a）**：输出第一行标当前轨道--Lite 标「[Lite·{Verdict/Fast/Formal/Follow-up}轨]」·至尊版标「[至尊版·议会+三道门]」·用户可知当前审核深度（对齐 rate-limit-communicator:101「降级须通知」律·消除静默降级双标）。
...
Lite 不内置圣人记忆系统（省令牌）。若需"同类问题第二次引用上次结论"的积累能力，转 sptler（孪生项目·见 README §孪生项目）（`/sptler`）——它是带 22 位圣人记忆的独立议会 skill，专利/技术决策场景更强；设计场景用 Lite 四轨即可，记忆非必需。

> **⚠️ 圣人体系不同（v4.2.7 R25 · 跨 skill 边界声明）**：suanfish-lite 的迷你 3 圣人（黑格尔/莫奈/倪瓒，思想家板凳）与 sptler 的 22 位圣人（邹蕴/王升/张鑫等真人专家）是**两套独立体系，无重叠**——转 sptler 不带 lite 的辩证人格连续性（黑格尔的辩证立场不会在 sptler 出现）。两 skill 场景分治：suanfish-lite 设计辩证 / sptler 专利技术决策。跨 skill 转换 = 换场景换人格，非续议。若需设计场景的圣人记忆积累，目前 suanfish 至尊版/Lite 均不内置，属未来 v4.3 议题（R25/R27 留的缺口 · 见 ref 50 §0）。
...
- **🔴 R 编号术语表不加载（v4.2.7 R27）**：Lite 不加载 ref 17（含 v4.2.7 哲理审计 R3 厘清的「R1-R25 哲学命题 vs R-01 审计规则」术语表 + R7-R12 空洞声明）。Lite agent 跑 ref 15/16 时按其 `R-XX`（连字符·审计规则集）处理，**勿与哲学命题 `R8`（无连字符·R7-R12 是编号空洞·不存在）混淆**。若 Lite 模式遇用户引裸 `R8`/`R10` 等，视为悬空（R7-R12 空洞），不当作存在规则——需核 ref 17 §三 R 全景（手动加载）确认。
